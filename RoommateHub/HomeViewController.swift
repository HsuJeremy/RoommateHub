...
class HomeViewController: UIViewController {
  ...
  override func viewDidLoad() {
    ...
    ref.child(roomIdentifier!).child("users").child(
  ...
  // Pass roomIdentifier to RoommateListViewController, MessagesListViewController,
  // and TaskListViewController
  override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
    if segue.identifier == "homeToRoommateList",
       let destination = segue.destination as? RoommateListViewController {
      destination.roomIdentifier = roomIdentifier
    } else if segue.identifier == "homeToMessageList",
              let destination = segue.destination as? MessagesListViewController {
      destination.roomIdentifier = roomIdentifier
