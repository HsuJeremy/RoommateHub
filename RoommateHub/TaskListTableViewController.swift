    ...
    ref.child(roomIdentifier!).child("taskList").observe(.value, with: { snapshot in
      ...
      for (key, value) in tasks {
        ...
        if checkCompleted == "false" {
          // If checkCompleted is false then add it to the array that will be used to show up on
          // this page
          let id = key
  ...
  override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
    ...
    if segue.identifier == "isCompletedSegue",
      ...
      destination.roomIdentifier = roomIdentifier
              ...
              let destination = segue.destination as? TaskAddViewController {
      destination.roomIdentifier = roomIdentifier
      let counter = Date().timer()
      destination.counter = counter!
    }
    else if segue.identifier == "CompletedTaskListSegue",
            let destination = segue.destination as? CompletedTaskListTableViewController {
      destination.roomIdentifier = roomIdentifier
    }
  }
}
